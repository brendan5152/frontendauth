...
const errorMessageHandler = (error) => {
    const errorMap = {
        ...
        "OAuthAccountNotLinked" : "Account not linked to an account",
        "OAuthCallbackError" : "OAuth callback error",
        "OAuthCreateAccountNotSupported" : "Account creation not supported",
        "OAuthSignin" : "Error during OAuth signin",
        "EmailCreateAccount" : "Error creating account",
        "EmailSignin" : "Error signing in",
...
const handleAuth = async (e) => {
    ...
    signIn('credentials', {
        ...
        console.log(error)
        setPageState(old => ({...old, processing: false, error: error.message ?? "Something went wrong! A SAH"}))
    })
}
      ...
      <div className="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            ...
            {
                pageState.error !==  '' && <AlertCard message={errorMessageHandler(pageState.error)} />
            }
          </div>
        </div>
      </div>
